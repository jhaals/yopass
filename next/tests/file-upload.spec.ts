...
test.describe('File Upload', () => {
  ...
  test('should display upload form with default values', async ({ page }) => {
    ...
    await expect(page.locator('text=Drag & drop or click to choose a file')).toBeVisible();
    await expect(page.locator('text=File upload is designed for small files')).toBeVisible();
    ...
    await expect(page.locator('label:has-text("One-time download") input[type="checkbox"]')).toBeChecked();
    await expect(page.locator('label:has-text("Generate decryption key") input[type="checkbox"]')).toBeChecked();
    
    // Check custom password field is not visible by default
    await expect(page.locator('input[placeholder="Enter your password..."]')).not.toBeVisible();
    
    // Check submit button is disabled by default (no file selected)
    await expect(page.locator('button[type="submit"]')).toBeDisabled();
    await expect(page.locator('button[type="submit"]')).toContainText('Upload File');
  });
  ...
  test('should upload file with default settings', async ({ page }) => {
    ...
    await expect(page.locator('button[title="Copy one-click link"]')).toBeVisible();
  ...
  test('should upload file with custom password', async ({ page }) => {
    ...
    await page.fill('input[placeholder="Enter your password..."]', customPassword);
    ...
    await expect(page.locator('button[title="Copy one-click link"]')).not.toBeVisible();
    await expect(page.locator('text=One-click link')).not.toBeVisible();
    
    // Should show short link
    await expect(page.locator('button[title="Copy short link"]')).toBeVisible();
    await expect(page.locator('div:has-text("Short link")').first()).toBeVisible();
    
    // Should show the custom password in the decryption key section
    const passwordCode = page.locator('div:has-text("Decryption key")').locator('..').locator('code').last();
  ...
  test('should handle binary files', async ({ page }) => {
    ...
    await expect(page.locator(`text=${testFiles.binaryFile.name}`)).toBeVisible();
    await page.click('button[type="submit"]');
  ...
  test('should handle different expiration times', async ({ page }) => {
    ...
    await expect(page.locator('h2:has-text("Upload file")')).toBeVisible({ timeout: 10000 });
  ...
  test('should toggle one-time download setting', async ({ page, browserName }) => {
    await mockAPI.mockUploadFile(mockResponses.fileUploaded);
    ...
    await page.locator('form input[type="checkbox"]').nth(0).uncheck({ force: true }); // First form checkbox is one-time
  ...
  test('should replace file when new file is selected', async ({ page }) => {
    ...
    await expect(page.locator(`text=${testFiles.textFile.name}`)).not.toBeVisible();
  });
  ...
  test('should validate complete file upload payload structure with all settings', async ({ page, browserName }) => {
    await mockAPI.mockUploadFile(mockResponses.fileUploaded);
    ...
    await page.locator('form input[type="checkbox"]').nth(1).uncheck({ force: true }); // Second form checkbox is generate key
