...
func init() {
	...
	pflag.String("expiration", viper.GetString("expiration"), "Duration after which secret will be deleted [1h, 1d, 1w, 1m]")
...
func encrypt(in io.ReadCloser, out io.Writer) error {
	...
	if exp == 0 {
		return fmt.Errorf("Expiration can only be 1 hour (1h), 1 day (1d), 1 week (1w), or 1 month (1m)")
	}
